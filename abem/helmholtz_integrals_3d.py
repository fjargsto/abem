...
def l_3d_p(k, p, qa, qb, qc, p_on_element):
    if p_on_element:
        if k == 0.0:
            ab = qb - qa
            ac = qc - qa
...
def m_3d_p(k, p, qa, qb, qc, p_on_element):
    if p_on_element:
        return 0.0
    else:
        if k == 0.0:
...
def mt_3d_p(k, p, vecp, qa, qb, qc, p_on_element):
    if p_on_element:
        return 0.0
    else:
        if k == 0.0:
...
def n_3d_p(k, p, vecp, qa, qb, qc, p_on_element):
    if p_on_element:
        if k == 0.0:
            ab = qb - qa
            ac = qc - qa
